...
from shaped_reward import shape_reward, TIMESTEPS, HYPERPARAMS

# ------------------------------------------------------------
# SINGLE-AGENT WRAPPER (SB3 REQUIRED)
# ------------------------------------------------------------

class SingleAgentEnv(gym.Env):
    """
    Exposes ONE agent to SB3 while simulating full PettingZoo env internally.
    Other agents are controlled by fixed/frozen policies or random.
    """

    def __init__(self, agent_id: str, other_models: dict = None):
        super().__init__()
    ...
    def step(self, action):
        ...
        # ----------------------------------------------------
        # other agents (frozen policies or random)
        # ----------------------------------------------------
        for a in self.env.agents:
            if a == self.agent_id:
                continue

            if a in self.other_models and self.other_models[a] is not None:
                obs = self._last_obs[a][None]
                act, _ = self.other_models[a].predict(obs, deterministic=False)
                actions[a] = act[0]
            else:
        ...
        done = terms.get(self.agent_id, False) or truncs.get(self.agent_id, False)

        reward = rewards.get(self.agent_id, 0.0)
        reward = shape_reward(obs[self.agent_id], reward)
...
def make_env(agent_id, other_models):
    def _init():
        return SingleAgentEnv(agent_id, other_models)
    return VecMonitor(DummyVecEnv([_init]))


# ------------------------------------------------------------
# TRUE IPPO TRAINING LOOP
# ------------------------------------------------------------

def train_ippo(total_timesteps=TIMESTEPS, n_rounds=3):

    run_dir = Path("runs") / "ippo_spread"
    run_dir.mkdir(parents=True, exist_ok=True)

    agent_ids = ["agent_0", "agent_1", "agent_2"]

    models = {a: None for a in agent_ids}

    timesteps_per_agent = total_timesteps // (n_rounds * len(agent_ids))

    for round_idx in range(n_rounds):

        print(f"\n===== IPPO Round {round_idx + 1} =====")

        for agent_id in agent_ids:

            print(f"\nTraining {agent_id}")

            # frozen snapshot of others
            frozen = {a: m for a, m in models.items() if a != agent_id}

            env = make_env(agent_id, frozen)
            ...
            models[agent_id].save(run_dir / f"{agent_id}_final")
