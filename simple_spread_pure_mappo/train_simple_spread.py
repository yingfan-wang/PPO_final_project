...
from simple_spread_pure_mappo.config import PureMAPPOConfig, RUN_PREFIX
from simple_spread_pure_mappo.env import (
...
def parse_args() -> PureMAPPOConfig:
...
def validate_config(config: PureMAPPOConfig) -> None:
...
def main() -> None:
    ...
    effective_timesteps = (
        (
            (
                config.timesteps
                + config.rollout_steps * config.num_envs
                - 1
            )
            // (config.rollout_steps * config.num_envs)
        )
    ...
    run_config["curriculum_switch_step"] = (
        curriculum_switch_step if config.curriculum else None
    )
    run_config["method"] = (
        "Pure MAPPO-style decentralized-actor centralized-critic PPO. The "
        "shared actor outputs primitive environment actions directly from each "
        "agent's local observation, and the critic evaluates per-agent values "
        "using the full global state and optional one-hot agent IDs."
    )
    run_config["actor_input"] = (
        "One local observation per agent, optionally concatenated with a one-hot "
        "agent identity."
    )
    run_config["critic_input"] = (
        "The fixed-order 54D global state from env.state(), repeated per agent "
        "and optionally concatenated with a one-hot agent identity."
    )
    run_config["credit_assignment"] = (
        "Primitive actions are learned directly with a centralized critic over "
        "the full team state, but no assignment prior or low-level controller "
        "is used."
    )
    ...
    def run_evaluation(eval_seed: int) -> float:
        ...
        plot_eval_curve(
            ...
            title="Pure MAPPO on Simple Spread",
        )
        ...
        print(
            f"[pure mappo eval] env_steps={env_steps} "
    ...
    try:
        while env_steps < config.timesteps:
            ...
            for _ in range(config.rollout_steps):
                ...
                critic_observations = build_critic_inputs(
                    joint_states,
                    use_agent_id=config.use_agent_id,
                )
            ...
            if update_idx == 0 or (update_idx + 1) % config.log_interval == 0:
                ...
                print(
                    f"[pure mappo update {update_idx + 1}] env_steps={env_steps} "
